...
$files = scandir("../../assets/images/".$dir);

//print_r($files);
//die();

try{
    $items=getItems($files,$dir);
}catch (Exception $e){
    echo $e->getMessage();
    return false;
}

$items['mediaFiles']&&$result['mediaFiles'] = $items['mediaFiles'];
$items['mediaFolders']&&$result['mediaFolders'] = $items['mediaFolders'];

$galleryItems = json_encode($result);
print_r($galleryItems);


function getItems($files, $dir){
    if(!isset($files) || $files== null || $files<0){throw new Exception('no files in specified directory');}
    $items = array();
    foreach($files as $file) {
        $path = "../../assets/images/".$dir."/".$file;
        $img =[];
        $folder =[];
        ...
        $img&&$img['dir'] = $dir. "/" .$file;
        $img&&$mediaFiles[] = $img;
        $folder&&$mediaFolders[] = $folder;
    }
    $mediaFiles&&$items['mediaFiles'] = $mediaFiles;
    $mediaFolders&&$items['mediaFolders'] = $mediaFolders;
    return $items;
}
